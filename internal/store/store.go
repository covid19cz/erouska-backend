...
func (i Client) Doc(path string) *firestore.DocumentRef {
	client := firebase.FirestoreClient
	return client.Collection(constants.CollectionRegistrations).Doc(path)
}
...
// FromContext extract store from context
...
// Doc returns a DocumentRef that refers to the document in the collection with the given identifier.
func (i MockClient) Doc(path string) *firestore.DocumentRef {
